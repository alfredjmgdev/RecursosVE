...
import { Truck, Package, CheckCircle2, Navigation, ShieldAlert } from 'lucide-react';
...
export const DriverDashboardView: React.FC = () => {
  ...
  // Persistent Leaflet map initialization (runs once map container is ready)
  useEffect(() => {
    if (!mapContainerRef.current || mapInstanceRef.current) return;

    const map = L.map(mapContainerRef.current, {
      zoomControl: true,
    }).setView([10.601, -66.932], 12);
  ...
  useEffect(() => {
    ...
    layerGroup.addLayer(leg2);

    // Auto-fit camera bounds ONLY on initial load
    ...
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col h-[520px]">
